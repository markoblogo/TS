...
export function Markets({ title, subtitle, items, markers }: Props) {
  ...
  return (
    <section id="markets" className="grid gap-4 lg:grid-cols-[1.1fr_1fr] lg:items-start">
      <MarketsMap markers={markers} activeKey={activeRegion} onMarkerEnter={setActiveRegion} onMarkerLeave={() => setActiveRegion(null)} />
          ...
          {items.map((item, idx) => {
            ...
            return (
              <li key={item}>
                <button
                  type="button"
                  onMouseEnter={() => markerKey && setActiveRegion(markerKey)}
                  onMouseLeave={() => setActiveRegion(null)}
                  onFocus={() => markerKey && setActiveRegion(markerKey)}
